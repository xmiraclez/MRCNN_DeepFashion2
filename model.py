...
from krcnn.model import MaskRCNN
...
class DeepFashion2Config(Config):
    ...
    NAME = "main_deepfashion2_sgd_bs8_2"
    ...
    train_img_dir = "/home/test/data/train/image/"
    train_json_path = "/home/test/data/train/train.json"
    valid_img_dir = "/home/test/data/validation/image/"
    valid_json_path = "/home/test/data/validation/valid.json"
...
def train(model, config):
    ...
    model.train(dataset_train, dataset_valid,
                ...
                epochs=DeepFashion2Config.EPCHS,
                layers='heads')
...
if __name__ == "__main__":
    ...
    if args.command == "train":
        ...
        rcnn = MaskRCNN(mode='inference', model_dir=args.logs, config=TestConfig())
        rcnn.load_weights(args.weights,by_name=True)
        ...
        for file in files:
            print(file)
            img = skimage.io.imread(input_folder+'/'+file)
            results = rcnn.detect([img], verbose=1)
            r = results[0]
            visualize.display_instances(img, r['rois'], r['masks'], r['class_ids'], class_names, file, output_folder, r['scores'])
